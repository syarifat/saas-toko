            ...
            <form action="{{ route('produk.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="pemasok_id" class="mt-1 w-full rounded-md border-gray-300">
                        ...
                        @foreach ($pemasok as $pm)
                            <option value="{{ $pm->id }}" {{ old('pemasok_id') == $pm->id ? 'selected' : '' }}>{{ $pm->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="number" name="harga_beli" min="0" step="1" value="{{ old('harga_beli') }}" required
                            class="mt-1 w-full rounded-md border-gray-300">
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Harga Jual *</label>
                        <input type="number" name="harga_jual" min="0" step="1" value="{{ old('harga_jual') }}" required
                            class="mt-1 w-full rounded-md border-gray-300">
                    </div>
                    <div>
                ...
                <div>
                    <label class="block text-sm font-medium">Stok Awal (masuk ke etalase)</label>
