<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Karyawan Baru</h2>
    </x-slot>
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <form action="{{ route('karyawan.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="posisi" value="{{ old('posisi') }}" placeholder="misal: Kasir"
                        class="mt-1 w-full rounded-md border-gray-300">
                </div>
                <div>
                    ...
                    <select name="skema_gaji" id="skema_gaji" onchange="toggleGaji()" required
                        class="mt-1 w-full rounded-md border-gray-300">
                        <option value="harian" {{ old('skema_gaji', 'harian') === 'harian' ? 'selected' : '' }}>Harian</option>
                        <option value="bulanan" {{ old('skema_gaji') === 'bulanan' ? 'selected' : '' }}>Bulanan (pokok)</option>
                    </select>
                </div>
                <div id="field-harian">
                    <label class="block text-sm font-medium">Tarif Harian</label>
                    <input type="number" name="tarif_harian" min="0" step="1" value="{{ old('tarif_harian') }}"
                        class="mt-1 w-full rounded-md border-gray-300">
                </div>
                <div id="field-bulanan" class="hidden">
                    <label class="block text-sm font-medium">Gaji Pokok Bulanan</label>
                    <input type="number" name="gaji_pokok" min="0" step="1" value="{{ old('gaji_pokok') }}"
                        class="mt-1 w-full rounded-md border-gray-300">
                </div>
                <div>
                    <label class="block text-sm font-medium">Tanggal Masuk *</label>
                    <input type="date" name="tanggal_masuk" value="{{ old('tanggal_masuk', now()->toDateString()) }}" required
                        class="mt-1 w-full rounded-md border-gray-300">
                </div>
