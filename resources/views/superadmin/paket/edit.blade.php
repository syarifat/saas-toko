...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800">Edit Paket: {{ $paket->nama }}</h2>
            <p class="text-sm text-slate-500">Perbarui informasi paket dan susunan modul yang aktif.</p>
        </div>
        <a href="{{ route('superadmin.paket.index') }}" class="text-sm font-medium text-slate-600 hover:text-slate-900">← Kembali</a>
    </div>

    <form method="POST" action="{{ route('superadmin.paket.update', $paket) }}" class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="jenis" required class="w-full text-sm border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="custom" {{ old('jenis', $paket->jenis) === 'custom' ? 'selected' : '' }}>Custom Tier</option>
                    <option value="preset_1" {{ old('jenis', $paket->jenis) === 'preset_1' ? 'selected' : '' }}>Preset 1</option>
                    <option value="preset_2" {{ old('jenis', $paket->jenis) === 'preset_2' ? 'selected' : '' }}>Preset 2</option>
                    <option value="preset_3" {{ old('jenis', $paket->jenis) === 'preset_3' ? 'selected' : '' }}>Preset 3</option>
                </select>
            </div>

            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1">Harga per Bulan (Rp) <span class="text-rose-500">*</span></label>
                <input type="number" name="harga" value="{{ old('harga', (int)$paket->harga) }}" min="0" step="1000" required
                       class="w-full text-sm border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            ...
            <div>
                ...
                <select name="aktif" class="w-full text-sm border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="1" {{ old('aktif', $paket->aktif ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif (Bisa Dipilih)</option>
                    <option value="0" {{ old('aktif', $paket->aktif ? '1' : '0') == '0' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="deskripsi" rows="2"
                          class="w-full text-sm border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 space-y-3">
            ...
            @php
                $currentModulIds = $paket->modul->pluck('id')->toArray();
            @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach($moduls as $modul)
                    @php
                        $depIds = $modul->ketergantungan->pluck('id')->toArray();
                        $selected = in_array($modul->id, old('modul_ids', $currentModulIds));
                    @endphp
